...
public class PulToLeftViewGroupl<recyclerview> extends LinearLayout implements ViewTreeObserver.OnGlobalLayoutListener {
   /*
   限制加载更多时 左拉的位移
   * */
    private int Limit_Move_ScrollX  = -300;//左移为负数
        ...
        for (int i = 0; i < mMoveViews.size(); i++) {
            if (i < mMoveRects.size()) {
                if (mMoveRects.get(i) != null) {
                    ...
                    mMoveViews.get(indexAt).layout(mMoveRects.get(indexAt).left, mMoveRects.get(indexAt).top, mMoveRects.get(indexAt).right, mMoveRects.get(indexAt).bottom);
                }
            }
        }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                int scrollX = (int) (nowX - startY);
                 if (scrollX < Limit_Move_ScrollX){//限制移动大小
                     return super.dispatchTouchEvent(ev);
                 }
